...
SELECT id, product_id, bidder_id, bid_amount, created_at FROM bids WHERE id = $1 LIMIT 1
`

func (q *Queries) GetBidByID(ctx context.Context, id uuid.UUID) (Bid, error) {
...
SELECT id, product_id, bidder_id, bid_amount, created_at FROM bids WHERE product_id = $1
`
...
SELECT id, product_id, bidder_id, bid_amount, created_at FROM bids WHERE bidder_id = $1
`

func (q *Queries) GetBidsByUserID(ctx context.Context, bidderID uuid.UUID) ([]Bid, error) {
	rows, err := q.db.Query(ctx, getBidsByUserID, bidderID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
